...
dummy_cli()
{
    PYTHONPATH=${PYTHONPATH}:. python3 licomp_proprietary/__main__.py $*
}
...
check_ret()
{
    ...
    then
        ...
        echo "devel/licomp-proprietary --local $ARGS ; echo \$?"
        exit 1
    fi
}
...
test_verify()
{
    ...
    run_comp_test 0 "verify -il BSD-3-Clause -ol Proprietary-linked"

    # Success and incompatible
    run_comp_test 2 "verify -il GPL-2.0-only -ol Proprietary-linked"
